...
export async function twoWayAddFriend(PlayFabId: string, FriendPlayFabId: string) {
    ...
    // Payloads
    const firstFriendRequestPayload = { PlayFabId, FriendPlayFabId };
    const secondFriendRequestPayload = { PlayFabId: FriendPlayFabId, FriendPlayFabId: PlayFabId };
    ...
    try {
        ...
        await Promise.all([
          sendRequest(setTagUrl, setTagPayload(PlayFabId, FriendPlayFabId, tags.SentPending)),
          sendRequest(setTagUrl, setTagPayload(FriendPlayFabId, PlayFabId, tags.RecievePending))
        ]);
    
        // Second friend request and setting tags
        await sendRequest(addFriendUrl, secondFriendRequestPayload);
        await Promise.all([
          sendRequest(setTagUrl, setTagPayload(FriendPlayFabId, PlayFabId, tags.SentPending)),
          sendRequest(setTagUrl, setTagPayload(PlayFabId, FriendPlayFabId, tags.RecievePending))
        ]);
    
        console.log('Friend requests and tags set successfully');
        return { success: true };
      } catch (error: any) {
        console.error('Error in processing friend requests and tags:', error.message);
